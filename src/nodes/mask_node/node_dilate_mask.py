...
def image_process(image, kernel_size, iterations):
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ret, mask = cv2.threshold(image, 127, 255, cv2.THRESH_BINARY)
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    image = cv2.dilate(mask, kernel, iterations=iterations)
...
class Node(DpgNodeABC):
    ...
    def add_node(
        ...
        with dpg.node(
            ...
            # ヒステリシス
            # TODO: Add callback for able to setup only 1, 3, 5, 7 etc kernel sizes
            with dpg.node_attribute(
                ...
                dpg.add_slider_int(
                    ...
                    default_value=3,
                    min_value=self._kernel_size_min,
                    max_value=self._kernel_size_max,
                    callback=None,
                )
            with dpg.node_attribute(
                ...
                dpg.add_slider_int(
                    ...
                    width=small_window_w - 80,
                    default_value=1,
                    min_value=self._iterations_min,
                    max_value=self._iterations_max,
                    callback=None,
                )
            # 処理時間
            if use_pref_counter:
    ...
    def update(
        ...
        if frame is not None:
            frame = image_process(frame, kernel_size, iterations)
